...
def query_rag(query_text: str, db_path: str):
    ...
    context_text = "\n\n---\n\n".join(
        [
            doc.page_content
            for doc, _ in db.similarity_search_with_score(query_text, k=3)
        ]
    )
    ...
    cleaned_response = re.sub(
        r"^\s*Answer:.*$", "", response_text, flags=re.MULTILINE
    ).strip()

    reference_map = {}

    answer_sentences = [
        s.strip() for s in re.split(r"(?<=[.!?])\s+", cleaned_response) if s.strip()
    ]

    for i, sentence in enumerate(
        answer_sentences[:3]
    ):  # Limit to first 3 sentences for references
        ...
        if any(
            x in source_file.lower() for x in ["prologue", "table of contents", "index"]
        ):
            continue
        ...
        # Get chunk ID which contains page:chunk_index information
        chunk_id = meta.get("id", "")
        ref_text = f"page {page}"

        # Extract chunk index from the ID format: "source:page:chunk_index"
        if chunk_id and ":" in chunk_id:
            parts = chunk_id.split(":")
            if len(parts) >= 3:
                chunk_index = (
                    int(parts[-1]) + 1
                )  # Convert to 1-based indexing (chunk 1, 2, 3...)
                ref_text = f"page {page}, chunk {chunk_index}"
